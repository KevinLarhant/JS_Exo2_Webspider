...
function get_page(page_url){
  ...
  request({
     ...
     */
	console.log("***********************************************************");
	console.info('Information on the currently crawled page\'s HTTP header');
	console.info('Request date : '+http_client_response.headers.date);
	console.info('Page size : '+http_client_response.headers['content-length']);
	console.info('Server\'s web page : '+http_client_response.headers.server);
	console.info('Content-Type : '+http_client_response.headers['content-type']+'\n');
	console.log("***********************************************************");
...
var argsCommandLine = process.argv[2];

// #debug Start the crawler with a link
get_page(argsCommandLine);
